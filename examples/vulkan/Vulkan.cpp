...
public:
    ...
    ~VulkanExample()
    {
        ...
        for (VkFence fence : fences) vkDestroyFence(device, fence, 0);
        ...
        for (VkDeviceMemory i : uniformBuffersMemory) vkFreeMemory(device, i, 0);

        for (VkBuffer uniformBuffer : uniformBuffers) vkDestroyBuffer(device, uniformBuffer, 0);
    ...
    void cleanupSwapchain()
    {
        ...
        for (VkFence fence : fences) vkWaitForFences(device, 1, &fence, VK_TRUE, std::numeric_limits<uint64_t>::max());
        ...
        for (VkImageView swapchainImageView : swapchainImageViews) vkDestroyImageView(device, swapchainImageView, 0);

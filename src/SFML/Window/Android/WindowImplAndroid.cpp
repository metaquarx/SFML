...
: m_size(0, 0), m_windowBeingCreated(false), m_windowBeingDestroyed(false), m_hasFocus(false)
{
}
...
: m_size(mode.width, mode.height), m_windowBeingCreated(false), m_windowBeingDestroyed(false), m_hasFocus(false)
{
    ...
    switch (action)
    {
        ...
        case AKEY_EVENT_ACTION_MULTIPLE :
            ...
            else if (Uint32 unicode = static_cast<Uint32>(getUnicode(_event))) // This is a repeated sequence
            {
                ...
                for (int32_t i = 0; i < repeats; ++i) forwardEvent(event);
                return 1;
            }
            break;
    }

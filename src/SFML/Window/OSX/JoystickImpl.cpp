...
bool JoystickImpl::open(unsigned int index)
{
    ...
    for (IOHIDElementRef iohidElementRef : m_buttons) CFRetain(iohidElementRef);

    for (const auto& [axis, iohidElementRef] : m_axis) CFRetain(iohidElementRef);
...
void JoystickImpl::close()
{
    ...
    for (IOHIDElementRef iohidElementRef : m_buttons) CFRelease(iohidElementRef);

    m_buttons.clear();

    for (const auto& [axis, iohidElementRef] : m_axis) CFRelease(iohidElementRef);
...
JoystickCaps JoystickImpl::getCapabilities() const
{
    ...
    // Axis:
    for (const auto& [axis, iohidElementRef] : m_axis) caps.axes[axis] = true;
...
JoystickState JoystickImpl::update()
{
    ...
    if (m_hat != nullptr)
    {
        ...
        switch (raw)
        {
            case 1 :
            case 2 :
            case 3 : state.axes[Joystick::PovX] = +100; break;

            case 5 :
            case 6 :
            case 7 : state.axes[Joystick::PovX] = -100; break;

            default : state.axes[Joystick::PovX] = 0; break;
        }
        ...
        switch (raw)
        {
            case 0 :
            case 1 :
            case 7 : state.axes[Joystick::PovY] = +100; break;

            case 3 :
            case 4 :
            case 5 : state.axes[Joystick::PovY] = -100; break;

            default : state.axes[Joystick::PovY] = 0; break;
        }
    }

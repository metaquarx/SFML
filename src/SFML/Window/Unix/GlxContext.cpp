...
: m_display(nullptr), m_window(0), m_context(nullptr), m_pbuffer(0), m_ownsWindow(false)
{
...
void GlxContext::createSurface(GlxContext* shared, unsigned int width, unsigned int height, unsigned int bitsPerPixel)
{
    ...
    if (shared && SF_GLAD_GLX_SGIX_pbuffer)
    {
        ...
        if (hasCreatePbuffer)
        {
            ...
            {
                int attributes[] =
                    {GLX_PBUFFER_WIDTH, static_cast<int>(width), GLX_PBUFFER_HEIGHT, static_cast<int>(height), 0, 0};

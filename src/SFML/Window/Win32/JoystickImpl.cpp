...
JoystickState JoystickImpl::update()
{
    ...
    if (joyGetPosEx(m_index, &pos) == JOYERR_NOERROR)
    {
        ...
        // Buttons
        for (unsigned int i = 0; i < Joystick::ButtonCount; ++i) state.buttons[i] = (pos.dwButtons & (1u << i)) != 0;
    }
...
void JoystickImpl::updateConnectionsDInput()
{
    ...
    for (JoystickRecord& record : joystickList) record.plugged = false;
...
bool JoystickImpl::openDInput(unsigned int index)
{
    ...
    for (int& axis : m_axes) axis = -1;

    for (int& button : m_buttons) button = -1;
...
JoystickCaps JoystickImpl::getCapabilitiesDInput() const
{
    ...
    for (int i = 0; i < Joystick::AxisCount; ++i) caps.axes[i] = (m_axes[i] != -1);

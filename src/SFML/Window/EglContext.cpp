...
: m_display(EGL_NO_DISPLAY), m_context(EGL_NO_CONTEXT), m_surface(EGL_NO_SURFACE), m_config(nullptr)
{
...
: m_display(EGL_NO_DISPLAY), m_context(EGL_NO_CONTEXT), m_surface(EGL_NO_SURFACE), m_config(nullptr)
{
    ...
    sf::err() << "Warning: context has not been initialized. The constructor EglContext(shared, settings, width, "
                 "height) is currently not implemented."
              << std::endl;
}

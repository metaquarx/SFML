...
: m_buffer(0), m_samples(copy.m_samples), m_duration(copy.m_duration), m_sounds() // don't copy the attached sounds
{
    // Create the buffer
    alCheck(alGenBuffers(1, &m_buffer));
...
SoundBuffer::~SoundBuffer()
{
    ...
    for (Sound* soundPtr : sounds) soundPtr->resetBuffer();
...
bool SoundBuffer::update(unsigned int channelCount, unsigned int sampleRate)
{
    ...
    for (Sound* soundPtr : sounds) soundPtr->setBuffer(*this);
